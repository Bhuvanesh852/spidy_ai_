...
# On Vercel (and most serverless platforms) only /tmp is writable — the
# deployment bundle itself is read-only, so writing to BASE_DIR/"uploads"
# raises OSError: Read-only file system on the very first request.
IS_SERVERLESS = bool(os.environ.get("VERCEL") or os.environ.get("AWS_LAMBDA_FUNCTION_NAME"))
UPLOAD_DIR = Path("/tmp/spidy-uploads") if IS_SERVERLESS else BASE_DIR / "uploads"
